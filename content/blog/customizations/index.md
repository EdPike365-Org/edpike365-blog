...
[edpike365.com](http://edpike365.com) started with Kyle Mathews' simple [Gatsby Starter Blog](https://www.gatsbyjs.com/starters/gatsbyjs/gatsby-starter-blog). I used Gatsby after doing some research in late 2020. I almost used Next.js; it was a toss of a coin. After Vercel, the maker of Next.js, bought Svelte, I wish the coin had flipped the other way. I'm very interested in Svelte but have not had time to play with it.

The site (will be) available from github so you can see my code. My two Gatsby Plugins (below) are already available on NPM and github.

Here are my customizations and plugin installations, in rough order:

### - MD Blog Publishing Workflow

- I wrote code to make "gatsby build" not build blog entries that are not in the published mode. "gatsby develop" will build them so I can see what they'll look like.
- I documented how I built it in my blog entry: [Gatsby Page Queries](/gatsby-page-queries/).
- I wrote my own "next/last" blog post footer code to learn how exactly the existing one worked. Gatsby gives you so many "magic" plugins to choose from, but my main goal for this edpike365.com is learning, not speed of develop.
...
- I learned many things:
  - using an IIFE, which executes before page render, to prevent an initial flash when the user loads the site.
...
- I used Chrome tools emulators to design it to work **by default** with the narrowest old smart phone ( w < 320px ). I modify multiple things (fonts, icons, menus) as the width increases. Check it out by playing with the width.
...
- TODO: implement SPA and offline mode.
- TODO: in mobile sizes make most navigation "thumb friendly" by moving it to the bottom, leave logo and user icon on top.

### - Implemented Emotion CSS in JS

- Learning to customize styling in things like Gatsby's **Image** tag was challenging.
- TODO: document gotcha details in a blog post.

### - Other CSS Customizations

- I added CSS Grid and Flexbox.
...
- I implemented CSS variables to support dynamic themeing.
- TODO: Add a footer that is attached to the bottom of all content, but not sticky.
- TODO: I'm still getting duplicate scroll bar issues with some MD blog entries. My hunch is its Graphviz (flow diagrams) or KaTex (math notation) content, which they both have exclusively in common. (my unpublished [STEM MD](/gatsby-remark-stem/) and [Gatsby Remark Test](/gatsby-remark-test/) blog entries).
